...
void ti_set_default_fp(taichi::lang::CompileConfig* config, int primitive_type_id) {  
  TI_CAPI_TRY_CATCH_BEGIN();  
  TI_CAPI_ARGUMENT_NULL(config);  
    
  // 验证输入的类型ID是否有效  
  if (primitive_type_id < 0) {  
    ti_set_last_error(TI_ERROR_INVALID_ARGUMENT, "Invalid primitive type ID");  
    return;  
  }  
  ...
  // 验证是否为浮点类型  
  if (type_id != taichi::lang::PrimitiveTypeID::f16 &&   
      type_id != taichi::lang::PrimitiveTypeID::f32 &&   
      type_id != taichi::lang::PrimitiveTypeID::f64) {  
    ti_set_last_error(TI_ERROR_INVALID_ARGUMENT, "Type must be a floating point type");  
    return;  
  }  
    
  // 直接设置配置中的默认浮点类型  
  config->default_fp = data_type;  
    
  TI_CAPI_TRY_CATCH_END();  
}
void ti_set_default_ip(taichi::lang::CompileConfig* config, int primitive_type_id) {  
  TI_CAPI_TRY_CATCH_BEGIN();  
  TI_CAPI_ARGUMENT_NULL(config);  
  ...
  // 验证是否为浮点类型  
  if (type_id != taichi::lang::PrimitiveTypeID::i16 &&   
      type_id != taichi::lang::PrimitiveTypeID::i32 &&   
      type_id != taichi::lang::PrimitiveTypeID::i64) {  
    ti_set_last_error(TI_ERROR_INVALID_ARGUMENT, "Type must be a floating point type");  
    return;  
  }  
    
  // 直接设置配置中的默认浮点类型  
  config->default_ip = data_type;  
    
  TI_CAPI_TRY_CATCH_END();  
}

void ti_set_default_up(taichi::lang::CompileConfig* config, int primitive_type_id) {  
  TI_CAPI_TRY_CATCH_BEGIN();  
  TI_CAPI_ARGUMENT_NULL(config);  
...
int ti_get_primitive_type_by_name(const char* type_name) {  
  ...
  if (name == "f32") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::f32);  
  } else if (name == "f64") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::f64);  
  } else if (name == "f16") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::f16);  
  } else if (name == "i32") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::i32);  
  } else if (name == "i64") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::i64);  
  } else if (name == "i16") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::i16);  
  } else if (name == "i8") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::i8);  
  } else if (name == "u32") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::u32);  
  } else if (name == "u64") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::u64);  
  } else if (name == "u16") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::u16);  
  } else if (name == "u8") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::u8);  
  } else if (name == "u1") {  
    return static_cast<int>(taichi::lang::PrimitiveTypeID::u1);  
  } else {  
    ti_set_last_error(TI_ERROR_INVALID_ARGUMENT, "Unknown primitive type name");  
    return -1;  
  }  
